  ...
  Future<void> action(
    ...
    switch (value) {
      ...
      case TrackOptionValue.delete:
        await File((track as DeeMusiqLocalTrackObject).path).delete();
      ...
      case TrackOptionValue.details:
        if (track is! DeeMusiqFullTrackObject) break;
        showDialog(
          context: context,
          builder: (context) => ConstrainedBox(
            constraints: const BoxConstraints(maxWidth: 400),
            child: TrackDetailsDialog(track: track as DeeMusiqFullTrackObject),
          ),
        );
        break;
      case TrackOptionValue.download:
        if (track is DeeMusiqLocalTrackObject) break;
        downloadManager.addToQueue(track as DeeMusiqFullTrackObject);
        break;
...
final trackOptionActionsProvider =
    Provider.family<TrackOptionsActions, DeeMusiqTrackObject>(
  (ref, track) => TrackOptionsActions(ref, track),
);

final trackOptionsStateProvider =
    Provider.family<TrackOptionFlags, DeeMusiqTrackObject>((ref, track) {
  ...
  final isInDownloadQueue = playlist.activeTrack == null ||
          playlist.activeTrack! is DeeMusiqLocalTrackObject
      ? false
      : const [
